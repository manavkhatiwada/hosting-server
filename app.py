...
from models import db, Appointment, ChatHistory, DoctorTimeSlot
...
def appointment():
        ...
        try:
            ...
            try:
                ...
                if app.config['MAIL_SUPPRESS_SEND']:
                    # For development: auto-verify and provide verification link
                    new_appointment.verified = True
                    db.session.commit()
                    flash(f'Development mode: Email sending suppressed. Appointment auto-verified. Verification link: <a href="{link}">{link}</a>', 'info')
                else:
    ...
    return render_template('appointment.html', active_page='appointment', department=department, doctor=doctor, hospital=hospital)
...
Viral infection – such as a common cold or flu
...
Take paracetamol or ibuprofen to reduce fever and pain
...
Severe headache, rash, or swelling in the face or throat
...
Based on the user's symptoms, recommend the most appropriate department, doctor, and hospital. Use this format at the end of your response:

[RECOMMENDATION]
Department: [Department Name]
Doctor: [Doctor Name]
Hospital: [Hospital Name]
Reason: [Brief reason why this doctor is recommended]

Available departments and doctors:
- General: Dr. Smith (Norvic Hospital), Dr. Johnson (Civil Hospital)
- Cardiology: Dr. Williams (Norvic Hospital), Dr. Garcia (Frontline Hospital)
- Neurology: Dr. Brown (Norvic Hospital), Dr. Miller (Civil Hospital)
- Pediatrics: Dr. Davis (Frontline Hospital), Dr. Wilson (Norvic Hospital)
- Dermatology: Dr. Martinez (Civil Hospital), Dr. Anderson (Frontline Hospital)
- Orthopedics: Dr. Taylor (Norvic Hospital), Dr. Thomas (Civil Hospital)

Always include a [RECOMMENDATION] section when the user describes health symptoms that would benefit from medical consultation.
...
"""
    ...
    recommendation = None  # Ensure recommendation is always defined
    try:
        # Get response from Groq
        completion = groq_client.chat.completions.create(
            model="gemini-1.5",  # or another suitable model
            messages=messages,
            temperature=0.7,
            max_tokens=1024
        )
        ...
        if '[RECOMMENDATION]' in response:
            try:
                ...
                start_idx = response.find('[RECOMMENDATION]')
                end_idx = response.find('\n\n', start_idx)
                if end_idx == -1:
                    end_idx = len(response)

                recommendation_text = response[start_idx:end_idx]

                # Parse department, doctor, and hospital
                ...
                if department and doctor:
                    recommendation = {
                        ...
                        'doctor': doctor,
                        'hospital': hospital,
                        'reason': reason or f"Based on your symptoms, {doctor} from {department} would be the best specialist to consult."
                    }
            except Exception as parse_error:
                print(f"Error parsing recommendation: {parse_error}")
...
def get_available_slots():
    try:
        ...
        if not doctor or not date:
            return jsonify({'error': 'Doctor and date are required', 'available_slots': []}), 400
        ...
        with app.app_context():
            time_slots = DoctorTimeSlot.query.filter_by(
                doctor_name=doctor,
